...
## 12. Информация по работе с удаленными репозиториями

### Отправить локальный репозиторий на удаленный

1. Создать аккаунт на GitHub.com.
2. Создать репозиторий в ЛК на GitHub.com
3. Создать локальный репозиторий. Для этого надо создать папку,
внутри неё вызвать команду git init, совершить какие-то действия, создать файлы, чтобы
появился хотя бы один коммит.

4. Связать локальный и удалённый репозитории. GitHub при создании нового
репозитория подскажет, как это можно сделать.

необходимые команды:

```
git remote add origin <адрес удаленного репозитория> -> команда привязка локального репозитория к удаленному

git branch -M main -> команда смены названия ветки master на название main

git push -u origin main -> команда которая отправляет изменения в удаленный репозиторий из локального; -u нужно только один раз, таким образом мы делаем ветку main активной
```

5. После посленей команды при первом использовании GitHub,
возможно, потребуется авторизоваться на удалённом репозитории. Сделать это надо
один раз, «подружить» редактор VS Code с GitHub, в дальнейшем эту операцию
проводить уже не понадобится.

6. В последствии необходимо только сохранять и комитить изменения в локальном репозитории и с помощью команды `git push` отправлять их в удаленный репозиторий

### Выгрузить удаленный репозиторий на компьютер


1. Провести изменения «с другого компьютера» или на GitHub
2. Выкачать с помощью команды `git pull` актуальное состояние из удалённого репозитория.

### Репозиторий для **pull request**
* В своём аккаунте на GitHub создать копию репозитория. Например (**"AndreyBulgakov19/SCV_Git_PR"**) с помощью кнопки **"Fork"**.
---
